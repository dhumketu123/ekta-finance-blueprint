...
-- 1️⃣ Drop existing conflicting functions to avoid signature issues
...
-- 2️⃣ Idempotency + Auto-Fix for duplicates
CREATE OR REPLACE FUNCTION public.assert_pipeline_idempotency(auto_fix boolean DEFAULT false)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    IF v_dup_count>0 THEN
        IF auto_fix THEN
            ...
            USING (
                SELECT MIN(ctid) as keep_ctid, entity_id, insight_type
                FROM public.ai_insights
                WHERE metadata->>'auto_generated'='true' AND status='active'
                GROUP BY entity_id, insight_type
            ) b
            WHERE a.entity_id=b.entity_id AND a.insight_type=b.insight_type AND a.ctid<>b.keep_ctid;
        ELSE
...
-- 3️⃣ Decision Engine (scoring + alerts)
...
-- 4️⃣ Main pipeline wrapper (atomic + metrics + auto-fix duplicates)
...
BEGIN
    ...
    PERFORM public.assert_pipeline_idempotency(true);
...
-- 5️⃣ Materialized view + unique index
...
-- 6️⃣ Safe refresh with isolated exception
...
VALUES (
    'v2.4.1',
    'lovable-architect',
    'Gap-Fixed: atomic pipeline, advisory lock, auto-fix duplicates, scoring validation, IMMEDIATE/HIGH alerts, safe mat-view refresh'
);
