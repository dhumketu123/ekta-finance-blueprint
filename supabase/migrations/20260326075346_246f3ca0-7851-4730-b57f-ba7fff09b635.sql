...
CREATE OR REPLACE FUNCTION public.apply_loan_payment(_loan_id uuid, _amount numeric, _performed_by uuid, _reference_id text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  ...
  _receipt_num text;
  _result jsonb;
  -- PHASE 6: Trust Score Engine variables
...
BEGIN
  ...
  -- Auto-reopen closed loans that still have debt
  ...
  IF _loan.status NOT IN ('active', 'overdue') THEN
  ...
  -- Collision-proof receipt number
