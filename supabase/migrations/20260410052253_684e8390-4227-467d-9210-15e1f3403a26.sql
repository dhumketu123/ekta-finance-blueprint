...
BEGIN
  ...
  LOOP
    ...
    -- 2️⃣ STRICT SUB-TRANSACTION BOUNDARY
    BEGIN
      -- ADVISORY LOCK
      ...
      -- 7️⃣ QUEUE FINALIZATION
      UPDATE public.ledger_guard_queue
      SET processed_at = now(), processing = false
      WHERE reference_id = v_rec.reference_id;

    EXCEPTION WHEN OTHERS THEN
      -- Per-record rollback — other records unaffected
      UPDATE public.ledger_guard_queue
