...
BEGIN
  ...
  -- PHASE 4: Generate collision-proof receipt number and insert financial_transactions
  _receipt_num := 'RCP-' || to_char(now(), 'YYYYMMDD-HH24MISS') || '-' || left(replace(gen_random_uuid()::text, '-', ''), 6);
