...
DROP FUNCTION IF EXISTS public.apply_loan_payment(uuid, numeric, uuid, text);
...
BEGIN
  ...
  UPDATE public.loans SET
    ...
    status = CASE WHEN (_total_outstanding - _amount) <= 0 THEN 'closed'::loan_status ELSE 'active'::loan_status END,
  ...
  UPDATE public.clients SET
    ...
    status = CASE WHEN (_total_outstanding - _amount) <= 0 THEN 'closed'::client_status ELSE status END,
    updated_at = now()
...
-- Fix trigger to check actual balance, not just status
