...
-- ═══════════════════════════════════════
-- PATCH 1: Unique index for dedup
-- ═══════════════════════════════════════
CREATE UNIQUE INDEX IF NOT EXISTS idx_ai_insights_unique_auto
ON public.ai_insights (entity_id, insight_type)
WHERE metadata->>'auto_generated' = 'true'
  AND status = 'active';

-- ═══════════════════════════════════════
-- PATCH 3 & 5: Rebuild materialized view with weighted_risk_score
-- ═══════════════════════════════════════
...
CREATE UNIQUE INDEX idx_ai_system_health_mat_single ON public.ai_system_health_mat ((1));
...
-- ═══════════════════════════════════════
-- PATCH 2, 4, 5: Rebuild fn_generate_ai_insights with all patches
-- ═══════════════════════════════════════
...
DECLARE
  ...
  rec record;
BEGIN
  -- PATCH 4: Auto-resolve stale insights older than 30 days
  WITH resolved AS (
  ...
  -- Clear previously auto-generated active insights (upsert handles rest)
  -- We resolve old auto-gen first so upsert can re-insert fresh ones
  UPDATE public.ai_insights
    SET status = 'resolved'
    WHERE status = 'active'
      AND metadata->>'auto_generated' = 'true';

  -- 1) High criticality entities → risk (UPSERT)
  FOR rec IN
  ...
  LOOP
    ...
    WHERE metadata->>'auto_generated' = 'true' AND status = 'active'
    DO UPDATE SET
  ...
  -- 2) Entities with >5 dependencies → dependency_warning (UPSERT)
  FOR rec IN
  ...
  -- 3) Inactive but still referenced → dependency_warning (UPSERT)
  FOR rec IN
  ...
  -- PATCH 2: Real circular dependency detection
  FOR rec IN
    WITH RECURSIVE dep_graph AS (
      ...
      UNION ALL
      SELECT d.source_entity_id, er.target_entity_id,
             d.path || er.target_entity_id
      FROM dep_graph d
      JOIN public.entity_relations er
        ON d.target_entity_id = er.source_entity_id
    ...
    JOIN public.entity_relations er2
      ON dg.target_entity_id = er2.source_entity_id
      AND er2.target_entity_id = dg.source_entity_id
  ...
  -- 4) High version churn → anomaly (UPSERT)
  FOR rec IN
