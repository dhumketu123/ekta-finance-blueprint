    ...
    <div className="relative w-full">
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-9 pr-3 py-2 rounded-lg border border-border bg-background text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-ring transition-shadow"
        />
      </div>
        ...
        <ul className="absolute z-50 mt-1 w-full max-h-60 overflow-auto rounded-lg border border-border bg-popover shadow-lg">
          {results.map((item, idx) => (
            ...
            >
              {renderItem
                ? renderItem(item)
                : String((item as Record<string, unknown>).title ?? (item as Record<string, unknown>).name ?? (item as Record<string, unknown>).email ?? "")}
            </li>
          ))}
        </ul>
      )}
