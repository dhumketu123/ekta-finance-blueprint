...
import {
  ...
  Bell, Settings, Shield, TrendingUp, Search, X, LogOut, FlaskConical, ClipboardCheck, BarChart3, ShieldAlert, Monitor, Crown, Atom, Handshake, Activity, Flame, ShieldCheck, Lock,
} from "lucide-react";
...
export const AppSidebar = () => {
  ...
  return (
    <>
      ...
      >
        {/* Brand */}
        <div className="p-5" style={{ borderBottom: "1px solid hsl(180 60% 22%)" }}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl flex items-center justify-center shadow-md" style={{ backgroundColor: "hsl(51 100% 50%)" }}>
                <Landmark className="w-5 h-5" style={{ color: "hsl(51 100% 10%)" }} />
              </div>
              <div>
                <h1 className="text-sm font-bold font-english tracking-tight" style={{ color: "hsl(0 0% 95%)" }}>{t("brand.name")}</h1>
                <p className="text-[10px] font-bangla" style={{ color: "hsl(180 15% 60%)" }}>{t("brand.tagline")}</p>
              </div>
            </div>
            <button onClick={close} className="p-1.5 rounded-lg transition-colors hover:bg-white/10" style={{ color: "hsl(180 15% 60%)" }}>
              <X className="w-4 h-4" />
            </button>
          </div>
        </div>
        ...
        <div className="px-3 pt-3 pb-1">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5" style={{ color: "hsl(180 15% 60%)" }} />
            <input
              type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
              placeholder={t("search.placeholder")}
              className="w-full pl-9 pr-8 py-2 rounded-lg text-sm focus:outline-none transition-all"
              style={{
                backgroundColor: "hsl(180 80% 20% / 0.4)",
                color: "hsl(0 0% 95%)",
                border: "1px solid hsl(180 60% 22% / 0.5)",
              }}
            />
            {searchQuery && (
              <button onClick={() => setSearchQuery("")} className="absolute right-2 top-1/2 -translate-y-1/2 hover:text-white" style={{ color: "hsl(180 15% 60%)" }}>
        ...
        <nav className="flex-1 py-2 px-3 space-y-1 overflow-y-auto">
          {filteredItems.length === 0 ? (
            <p className="text-xs text-center py-4 font-bangla" style={{ color: "hsl(180 15% 60%)" }}>{t("search.noResults")}</p>
          ) : (
            filteredItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  style={{
                    backgroundColor: isActive ? "hsl(180 80% 20%)" : "transparent",
                    color: isActive ? "hsl(51 100% 50%)" : "hsl(0 0% 95% / 0.7)",
                  }}
                  onMouseEnter={(e) => { if (!isActive) e.currentTarget.style.backgroundColor = "hsl(180 80% 20% / 0.5)"; e.currentTarget.style.color = "hsl(0 0% 95%)"; }}
                  onMouseLeave={(e) => { if (!isActive) e.currentTarget.style.backgroundColor = "transparent"; if (!isActive) e.currentTarget.style.color = "hsl(0 0% 95% / 0.7)"; }}
                >
                  {isActive && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-6 rounded-r-full" style={{ backgroundColor: "hsl(51 100% 50%)" }} />}
        ...
        <div className="p-4 space-y-3" style={{ borderTop: "1px solid hsl(180 60% 22%)", paddingBottom: "calc(1rem + env(safe-area-inset-bottom, 0px) + 80px)" }}>
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-xs font-medium truncate" style={{ color: "hsl(0 0% 95%)" }}>{user?.email || "User"}</p>
              <p className="text-[10px] capitalize" style={{ color: "hsl(180 15% 60%)" }}>{roleLabel}</p>
            </div>
          </div>
          <button
            onClick={handleLogout}
            className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-xs font-medium transition-colors"
            style={{ color: "hsl(0 100% 65%)", backgroundColor: "hsl(0 100% 65% / 0.1)" }}
          >
